...
class TARGET(object):
	def __init__(self, ATOM, M_X, pure = False, V_ESCAPE=V_ESCAPE, V_EARTH=V_EARTH, V_MEAN=V_MEAN, delta_keV=0):
		...
		delta_J = delta_keV * KEV
		
		self.V_MIN = lambda E_R: (
			(self.M_TAR * E_R / self.M_X_TAR + delta_J) / 
			np.sqrt(2 * self.M_TAR * E_R)
		)
